...
def recipeToJSON(browser, recipeID):
    ...
    recipe['ingredients'] = [re.sub(' +', ' ', li.text).replace('\n', '').strip() for li in
                             soup.select("#ingredients li")]
    recipe['steps'] = [re.sub(' +', ' ', li.text).replace('\n', '').strip() for li in
                       soup.select("#preparation-steps li")]
...
def run(webdriverfile, outputdir):
    ...
    reply = input('[CD] Do you want to dump all categories or specify filters by yourself (enter category or filter): ')
...
def dumpSite(brw, outputdir):
    ...
    for recipeURL in recipesURLs:
        ...
        brw.execute_script("arguments[0].setAttribute(arguments[1], arguments[2]);",
                           brw.find_element_by_class_name('core-tile__image'), 'srcset', '')
        brw.execute_script("arguments[0].setAttribute(arguments[1], arguments[2]);",
                           brw.find_element_by_class_name('core-tile__image'), 'src', local_img_path)
